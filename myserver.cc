...
int openSocket(void);
void sokcetBind(int socket_id, struct sockaddr_in srv_ad);
...
int main(int argc, char *argv[])
{
	...
	sokcetBind(s_sock_fd, srv_ad);	// binding socket
		...
		do
		{
			...
			else if (strncmp(buff, "_cre", 4) == 0)
			{
				...
				else
				{
					...
					{
						clearBuff(buff);
						string ack = "Server: File cannot be created!\n";
						strcpy(buff, ack.c_str());
						sendMsg(c_sock_fd, buff, strlen(buff));		
					}
